...
const EditRolePermission: React.FC<EditRolePermissionProps> = ({ roleId, onClose }) => {
  ...
  const [selectedPermissions, setSelectedPermissions] = useState<{ [menuId: number]: number[] }>({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const role = rolePermissionsData.find((r: RoleWithMenus) => r.role.id === parseInt(roleId || '0'));
        
        if (role) {
          setRoleData(role);
          
          const initialPermissions: { [menuId: number]: number[] } = {};
          interface MenuWithPermissions {
            id: number;
            permissionList: { id: number }[];
          }

          const roleMenuList: MenuWithPermissions[] = role.menuList;

          roleMenuList.forEach((menu: MenuWithPermissions) => {
            initialPermissions[menu.id] = menu.permissionList.map((p: { id: number }) => p.id);
          });
  ...
  const handlePermissionChange = (menuId: number, permissionId: number, checked: boolean) => {
