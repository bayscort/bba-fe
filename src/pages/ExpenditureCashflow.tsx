...
export default function ExpenditureCashflow() {
            ...
            try {
                ...
                if (selectedAccounts.length > 1) {
                    for (const account of selectedAccounts) {
                        const data = await fetchCashflow('expenditure', account.value, selectedYear);
                        newReports.push({
                ...
                if (selectedAccounts.length === 1) {
                    const data = await fetchCashflow('expenditure', selectedAccounts[0].value, selectedYear);
                    newReports.push({
                    ...
                    const data = await fetchCombineCashflow('expenditure', accountIds, selectedYear);
                    newReports.push({
    ...
    const onExport = () => {
        ...
        XLSX.writeFile(workbook, `Cashflow_Expenditure_${selectedYear}.xlsx`);
    };
